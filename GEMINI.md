...
- **Server Actions con `redirect()` y Analytics (GA4 / GTM):** En Server Actions que terminan en `redirect(...)` (como `createClass` o `updateClassFromForm`), Next.js lanza un error interno de control de flujo (`NEXT_REDIRECT`). Todo evento analítico disparado desde el cliente (`trackClassCreated`) debe capturarse en el bloque `catch` validando mediante `isRedirectError(err)` (comprobando tanto `message === 'NEXT_REDIRECT'` como `digest?.startsWith('NEXT_REDIRECT')`) antes de relanzar el error para no romper la navegación.
- **OpenGraph y Herencia de Metadatos en Páginas Hijas:** Al definir un objeto `openGraph` en una página hija (ej. `/blog` o `/blog/[slug]`), Next.js sobrescribe completamente el `openGraph` del layout raíz. Siempre deben declararse explícitamente `siteName: 'Kynea'`, `locale: 'es_PE'` y una imagen de respaldo (`BLOG_FALLBACK_IMAGE = `${SITE_URL}/img-portada-kynea.png``) para evitar previsualizaciones rotas en redes sociales y WhatsApp.
- **Esquema de Blog y Google Search Console:** Para datos estructurados `BlogPosting` o `Article`, Google exige el campo `image` (usar `BLOG_FALLBACK_IMAGE` como fallback para posts sin portada) y recomienda `publisher.logo` como `ImageObject` `{ '@type': 'ImageObject', url: ... }`.
- **Migraciones y Seeding entre Entornos (`kynea-dev` → `kynea-prod`):** Las inserciones de datos iniciales esenciales (ej. posts de blog publicados) deben estructurarse de forma estrictamente idempotente (`INSERT INTO ... ON CONFLICT (slug) DO NOTHING;`) dentro de migraciones versionadas en `supabase/migrations/`. La GitHub Action `supabase-prod-push.yml` aplica automáticamente `supabase db push` contra producción en cada push a `main`.
- **Numeración Secuencial de Migraciones en Supabase:** `develop` y `main` integran hasta `20260921000002_57_pgvector_ai_search.sql`. Nuevas migraciones locales deben continuar la secuencia estrictamente desde `58_` (`20260921000003_58_...`) para evitar colisiones de ordenamiento en Supabase.
- **Filtro de Categorías en Blog y Router Cache de Next.js:** En `/blog`, "Todas" es una etiqueta visual y no un valor en `blog_posts.category`. Cualquier query `?categoria=Todas` causará que `eq('category', 'Todas')` devuelva 0 posts a menos que se normalice a `undefined`. Asimismo, tras una migración de datos en vivo, el Router Cache del cliente puede servir la versión previa hasta un hard reload (`Cmd+Shift+R`) o revalidación con `revalidatePath('/blog')`.
- **Aislamiento con Git Worktrees para Releases y Hotfixes:** Cuando la rama local `develop` tenga trabajo en curso sin commitear, aislar siempre los releases a `main`, migraciones o hotfixes en worktrees hermanos (`<repo-parent>/<repo-name>-worktrees/<nombre>`) para no ensuciar ni arriesgar el working tree principal.
- **Búsqueda Semántica con IA y pgvector (Gemini & PostgreSQL):** Mapeo híbrido que combina afinidad coseno (`text-embedding-004` / 768d) con filtros duros en PostgreSQL vía RPC `match_classes_hybrid` (`SECURITY DEFINER SET search_path = public`). El parsing con Gemini Flash usa `SEARCH_RESPONSE_SCHEMA` con `temperature: 0.1` para decodificación restringida por gramática (inmune a prompt injection). Cuenta con rate limiting estricto de 10 req/min por IP en `proxy.ts`, sanitización con truncamiento estricto a 120 caracteres (`MAX_SEARCH_QUERY_LENGTH = 120`) y cache en memoria TTL de 2 horas (500 entradas) en Node.js para servir búsquedas idénticas en 0ms y $0 de costo.
- **Rendimiento de Entrada en Buscadores Móviles (Virtual Keyboard Lag Prevention):** En drawers o modales de búsqueda a pantalla completa en mobile (`MobileStyleSearchOverlay`, `MobileCitySearchOverlay`), el estado del input NUNCA debe residir en la raíz de un componente masivo (como `HomeClient` con decenas de tarjetas de clases y carruseles). Aislar siempre el estado localmente en el overlay para que las pulsaciones solo re-rendericen el input local. Aislar el contexto de composición con `isolate transform-gpu` para evitar repintados del fondo en WebKit/Blink móvil, usar flags nativos en `<input>` (`autoComplete="off"`, `autoCorrect="off"`, `autoCapitalize="none"`, `spellCheck={false}`, `enterKeyHint="search"`) y suspender timers y carruseles de fondo mientras el drawer móvil esté abierto.
