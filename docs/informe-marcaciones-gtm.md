...
- **`class_created`** — `trackClassCreated`, en el `catch` de `createClass(fd)` dentro de `handlePublish` (`CrearClaseForm.tsx`), capturando `isRedirectError(err)` (ya que `createClass` ejecuta `redirect()` en el servidor al tener éxito y lanza `NEXT_REDIRECT` en el cliente, de modo que código síncrono posterior a la llamada nunca se ejecutaba en publicaciones exitosas) — nunca para `updateClassFromForm` (edición). Parámetros: `status`, `class_type`, `class_style`.
