...
export default function ClasesMapView({ classes }: { classes: DanceClass[]; academias?: Teacher[] }) {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const itemRefs = useRef<Map<string, HTMLDivElement>>(new Map());

  const pins: MapPin[] = classes
    ...
    return (
      <div className="text-center py-24">
        ...
        <p className="text-neutral-500 text-[14px] max-w-sm mx-auto">
          Ninguna clase con estos filtros tiene una dirección con coordenadas todavía.
        </p>
      </div>
    );
  }
